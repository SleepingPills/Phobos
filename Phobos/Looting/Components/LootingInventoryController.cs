...
namespace Phobos.Looting.Components
{
    ...
    public class LootingInventoryController
    {
        ...
        public LootingInventoryController(BotOwner botOwner, LootingBrain lootingBrain)
        {
            try
            {
                _log = new BotLog(LootingSystem.LootLog, botOwner);
                _lootingBrain = lootingBrain;
                _itemAppraiser = LootingSystem.ItemAppraiser;
        ...
        */
        public async Task<bool> TryAddItemsToBot(IEnumerable<Item> items)
        {
            foreach (Item item in items)
            {
                ...
                {
                    if (LootingSystem.UseExamineTime.Value)
                    {
                    ...
                    else if (item is Weapon weapon && LootingSystem.CanStripAttachments.Value)
                    {
        ...
        */
        public bool ShouldSwapGear(Item equipped, Item itemToLoot)
        {
            ...
            {
                int equippedSize = LootUtils.GetContainerSize(equipped as SearchableItemItemClass);
                int itemToLootSize = LootUtils.GetContainerSize(itemToLoot as SearchableItemItemClass);
        ...
        */
        public bool IsValuableEnough(float itemPrice)
        {
            ...
            float min = (isPMC ? LootingSystem.PMCMinLootThreshold : LootingSystem.ScavMinLootThreshold).Value;
            float max = (isPMC ? LootingSystem.PMCMaxLootThreshold : LootingSystem.ScavMaxLootThreshold).Value;
        ...
        {
            Utilities.EquipmentType eligiblePmcGear = (Utilities.EquipmentType)LootingSystem.PMCGearToEquip.Value;
            Utilities.EquipmentType eligibleScavGear = (Utilities.EquipmentType)LootingSystem.ScavGearToEquip.Value;
        ...
        public bool AllowedToPickup(Item lootItem)
        {
            ...
            bool pickupNotRestricted = isPMC
                ? LootingSystem.PMCGearToPickup.Value.IsItemEligible(lootItem)
                : LootingSystem.ScavGearToPickup.Value.IsItemEligible(lootItem);

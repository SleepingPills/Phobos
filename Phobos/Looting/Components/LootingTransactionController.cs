...
// using InventoryControllerResultStruct = GStruct454; // Removed for dynamic compatibility
...
namespace Phobos.Looting.Components
{
    public class LootingTransactionController(InventoryController inventoryController, BotLog log)
    {
        ...
        {
            // Fika/SPT 4.0 Compatibility: Disabled ammo adding for now to avoid GStruct issues
            return true;
            /*
            try
            {
        ...
        {
            return inventoryController.TryRunNetworkTransaction((dynamic)operationResult, callback);
        }
        ...
        public static Task SimulatePlayerDelay(float delay = -1f)
        {
             if (delay == -1)
            {
                delay = LootingSystem.TransactionDelay.Value;
            }

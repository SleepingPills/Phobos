...
namespace Phobos.Looting.Components
{
    ...
    public class LootingBrain : MonoBehaviour
    {
            ...
            get
            {
                return ForceBrainEnabled
                    || (
                        ...
                        && (
                            LootingSystem.ContainerLootingEnabled.Value.IsBotEnabled(this)
                            || LootingSystem.LooseItemLootingEnabled.Value.IsBotEnabled(this)
                            || LootingSystem.CorpseLootingEnabled.Value.IsBotEnabled(this)
                        )
                    );
            }
        }
        ...
        {
            get { return Math.Pow(LootingSystem.LimitDistanceFromPlayer.Value, 2); }
        }
            ...
            try
            {
                if (BotOwner.BotState == EBotState.Active)
                {
                    if (ActiveBotCache.IsCacheActive && _performanceTimer < Time.time)
                    {
                        ...
                        _performanceTimer =
                            Time.time + Math.Min(PeformanceTimerInterval, LootingSystem.LootScanInterval.Value);
                    }
        ...
        */
        public IEnumerator LootCorpse()
        {
            if (ActiveCorpse != null)
            {
                ...
                if (LootingSystem.ShowLootingNotifications.Value && lootTask.Result)
                {
                    _log.LogInfo($"[{BotOwner.Profile.Nickname}] Looted corpse: {ActiveCorpse.Profile.Nickname}");
                }
        ...
        */
        public IEnumerator LootContainer()
        {
            ...
            if (LootingSystem.BotsAlwaysCloseContainers.Value || !didOpen)
            {
            ...
            if (LootingSystem.ShowLootingNotifications.Value && lootTask.Result)
            {
                 _log.LogInfo($"[{BotOwner.Profile.Nickname}] Looted container: {ActiveContainer.name.Localized()}");
            }
        ...
        */
        public IEnumerator LootItem()
        {
            if (ActiveItem?.ItemOwner?.RootItem != null)
            {
                ...
                if (LootingSystem.ShowLootingNotifications.Value && lootTask.Result)
                {
                     _log.LogInfo($"[{BotOwner.Profile.Nickname}] Picked up: {item.Name.Localized()}");
                }
        ...
        */
        ...
        {
            float itemValue = LootingSystem.ItemAppraiser.GetItemPrice(lootItem);

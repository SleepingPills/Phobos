...
namespace Phobos.Looting.Components
{
    public class LootFinder : MonoBehaviour
    {
        ...
        {
            get { return LootingSystem.DetectCorpseDistance.Value; }
        }
        private float DetectContainerDistance
        {
            get { return LootingSystem.DetectContainerDistance.Value; }
        }
        private float DetectItemDistance
        {
            get { return LootingSystem.DetectItemDistance.Value; }
        }
        ...
        {
            ScanTimer = Time.time + LootingSystem.InitialStartTimer.Value;
            _botOwner = botOwner;
            _lootingBrain = _botOwner.GetPlayer.gameObject.GetComponent<LootingBrain>();
            _log = new BotLog(LootingSystem.LootLog, _botOwner);
            LootingSystem.ActiveLootFinders.Add(this);
            _log.LogInfo($"[LootFinder] Initialized for {_botOwner.Profile.Nickname}");
        }

        public void OnDestroy()
        {
            LootingSystem.ActiveLootFinders.Remove(this);
        }
        ...
        public void ResetScanTimer()
        {
            ...
            {
                ScanTimer = Time.time + LootingSystem.LootScanInterval.Value;
            }
        }
        ...
        public IEnumerator FindLootCoroutine()
        {
            IsScanRunning = true;
                ...
                var containerLootingEnabled = LootingSystem.ContainerLootingEnabled.Value.IsBotEnabled(_lootingBrain);
                var itemLootingEnabled = LootingSystem.LooseItemLootingEnabled.Value.IsBotEnabled(_lootingBrain);
                var corpseLootingEnabled = LootingSystem.CorpseLootingEnabled.Value.IsBotEnabled(_lootingBrain);

                if (!containerLootingEnabled && !itemLootingEnabled && !corpseLootingEnabled)
                {
                    IsScanRunning = false;
                    yield break;
                }
                ...
                try
                {
                    ...
                    if (LootingSystem.ShowLootingNotifications.Value)
                    {
                       _log.LogInfo($"[{_botOwner.Profile.Nickname}] Loot Pulse: Found {hits} interactables within {detectionRadius}m");
                    }
        ...
        */
        ...
        public bool IsLootInSight(LootType lootType, Vector3 destination)
        {
            ...
            if (
                LootingSystem.DetectContainerNeedsSight.Value == false && isContainer
                || LootingSystem.DetectItemNeedsSight.Value == false && isItem
                || LootingSystem.DetectCorpseNeedsSight.Value == false && isCorpse
            )
            {
                return true;
            }
            ...
            Vector3 start = _botOwner.GetPlayer.PlayerBones.Head.position;

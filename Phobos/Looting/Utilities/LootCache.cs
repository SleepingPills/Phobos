    ...
    public static class ActiveLootCache
    {
        ...
        public static void Cleanup(BotOwner botOwner)
        {
            try
            {
                ...
                {
                    if (LootingSystem.LootLog.ErrorEnabled)
                    {
                        LootingSystem.LootLog.LogError("Cleanup issued on a bot with no name?");
                    }
                    return;
                }
                ...
                foreach (KeyValuePair<string, BotOwner> keyValue in ActiveLoot)
                {
                    ...
                    if (keyValue.Value == null || keyValue.Value.name == null)
                    {
                        if (LootingSystem.LootLog.ErrorEnabled)
                        {
                            LootingSystem.LootLog.LogError("Bot in loot cache has no name?");
                        }
                        continue;
                    }
            ...
            {
                LootingSystem.LootLog.LogError(e);
            }
        }
    }
}
